<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@page import="com.emergentes.Categoria"%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
           <link rel="stylesheet" href="css/style.css" type="text/css" >
        <title>JSP Page</title>
    </head>
    <body id="cat">
        <div id="categorias-edit">
       <h1>
            <c:if test="${requestScope.op=='grabar'}" var="res1" scope="request" >    
                Registro de 
            </c:if>
            <c:if test="${requestScope.op=='editar'}" var="res1" scope="request" >    
                Modificar
            </c:if>
            Categoria
        </h1>
         <jsp:useBean id="cates" scope="request" class="com.emergentes.Categoria"/> 
         <form action="CategoriaController" method="post">
             <table id="table">
               <tr>
                    <td>Id:</td>
                    <td><input type="text" name="id" value="<jsp:getProperty name="cates" property="id" />"></td>

                </tr>
                <tr>
                    <td>Categoria:</td>
                    <td><input type="text" name="categoria" value="<jsp:getProperty name="cates" property="categoria" />"></td>

                </tr>   
                <tr>
                    <td> 
                        <input type="hidden" name="op" value="${requestScope.op}"/>
                       
                    </td> 
                    <td><input type="submit" value="Enviar"></td>
                </tr>
                 
     </table>  
      </form>  
                        <p><a href="categorias.jsp">Volver</a></p>
     </div>
    </body>
</html>
